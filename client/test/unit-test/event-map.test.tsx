...
interface Event {
  ...
  _embedded: {
    venues: {
      name: string;
      location: {
        longitude: string; // Updated to string
        latitude: string; // Updated to string
      };
    }[];
  };
  dates: {
...
describe("EventMap Component", () => {
  const mockEvents: Event[] = [
    {
      ...
      _embedded: {
        venues: [
          {
            name: "Venue 1",
            location: { longitude: "10.7522", latitude: "59.9139" },
          },
        ],
      },
      dates: {
  ...
  const mockLocation: Location = { longitude: "10.7522", latitude: "59.9139" };

  it("renders map container", () => {
    const { container } = render(
      <EventMap events={mockEvents} location={mockLocation} />
    );
  ...
  it("initializes map with correct location", () => {
    ...
    expect(mapboxgl.Map).toHaveBeenCalledWith({
      ...
      center: [
        parseFloat(mockLocation.longitude),
        parseFloat(mockLocation.latitude),
      ],
      zoom: 8,
