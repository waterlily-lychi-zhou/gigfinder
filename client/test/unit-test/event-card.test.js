...
describe('EventCard Component', () => {
  ...
  const renderWithFavouritesContext = (favourites = []) => {
    render(
      <BrowserRouter>
  ...
  it('renders event details correctly', () => {
    ...
    expect(screen.getByText(mockEvent.dates.start.localDate)).toBeInTheDocument();
    expect(screen.getByText(mockEvent.dates.start.localTime)).toBeInTheDocument();
    expect(screen.getByText(mockEvent.venue)).toBeInTheDocument();
    expect(screen.getByAltText(mockEvent.name)).toHaveAttribute('src', mockEvent.images[0].url);
  });
  ...
  it('shows "Remove from Favourites" button when event is a favourite', () => {
    renderWithFavouritesContext([{ eventId: '1' }]);
